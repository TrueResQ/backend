...
export async function up(knex: Knex): Promise<void> {
  ...
  return knex.schema.createTable("user", (usersTable) => {
    ...
    usersTable.string("recovery_address", 127).notNullable().defaultTo("");
    usersTable.string("nominee", 127).notNullable().defaultTo("");
    usersTable.string("gaurdians", 1000).notNullable().defaultTo("");
